<?php
   ...
   define("INTENT_BACKFILE", $intent_backfile);
   ...
   if ($link) {
      $str_process_status_select = "select * from process_status where app_id = '{$appid}' and module = 'intent' order by id desc limit 1";
      if ($result = mysqli_query($link, $str_process_status_select)) {
         $num = mysqli_num_rows($result);
         if ($num == 1) {
            $row = mysqli_fetch_assoc($result);
            $end_at = $row["end_at"];
            $entity_file_name = $row["entity_file_name"];
            $entity_file_name = trim($entity_file_name);
            $array_entity_file_name = explode(":", $entity_file_name);
            if (!is_array("download", $array_entity_file_name)) {
               $entity_file_name = "";
            }
            if (mb_strlen(trim($entity_file_name)) == 0) {
               ...
               if (strcmp($row["status"], "success") == 0) {
                  $json_data = json_encode(array("return" => 0, "return_message" => "下载成功！", "end_at" => $end_at), JSON_UNESCAPED_UNICODE);
               } elseif (strcmp($row["status"], "running") == 0) {
                  $json_data = json_encode(array("return" => 0, "return_message" => "下载执行中！", "end_at" => $end_at), JSON_UNESCAPED_UNICODE);
               } elseif (strcmp($row["status"], "fail") == 0 || strcmp($row["status"], "rerun") == 0) {
                  $json_data = json_encode(array("return" => -1, "return_message" => "{$row["message"]}", "end_at" => $end_at), JSON_UNESCAPED_UNICODE);
               } else {
                  $json_data = json_encode(array("return" => 0, "return_message" => "", "end_at" => $end_at), JSON_UNESCAPED_UNICODE);
